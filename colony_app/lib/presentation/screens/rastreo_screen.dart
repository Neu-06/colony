...
class _RastreoScreenState extends State<RastreoScreen> {
  ...
  Future<void> _verificarSuscripcion() async {
    ...
    if (mounted) {
      setState(() {
        _yaSuscrito = suscripciones.contains(_tracking!.codigo);
      });
    }
  }
  ...
  Future<void> _solicitarNotificaciones() async {
    ...
    try {
      ...
      // Guardar localmente para no volver a pedir
      final prefs = await SharedPreferences.getInstance();
      final suscripciones = prefs.getStringList('suscripciones') ?? [];
      if (!suscripciones.contains(_tracking!.codigo)) {
        suscripciones.add(_tracking!.codigo);
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Te avisaremos cuando este trámite avance.',
                    style: GoogleFonts.inter(
                      fontSize: 12,
                      color: const Color(0xFF166534),
                    ),
